...
class Aeropuerto:
    ...
    def asigna_slot(self, vuelo) -> pd.Series:
        ...
        fecha_vuelo = vuelo.get('fecha_llegada')
        if fecha_vuelo is None:
            raise ValueError(f"El vuelo {vuelo.get('id', 'desconocido')} no tiene definida la 'fecha_llegada'.")
        ...
        print('el vuelo {} con fecha de llegada y despegue {}, {} ha sido asignado al slot {}'
              .format(vuelo['id'], vuelo['fecha_llegada'], vuelo['fecha_despegue'], slot))
        vuelo['slot'] = slot
        return vuelo 
